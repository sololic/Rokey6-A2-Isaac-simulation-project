...
"""
══════════════════════════════════════════════════════════════════
JACKAL + UR10  지상 테스트 데모  (Ground Test Mode)
══════════════════════════════════════════════════════════════════
수중 환경(파도/부력/댐핑/버블) 없음 → 팔 제어/균열 추종 빠른 테스트

실행:
  /home/rokey/isaacsim/python.sh jackal_ur10_ground_demo.py

파이프라인 (터미널 분리 실행):
  python3 camera/crack_detection_node.py
  python3 camera/crack_to_socket.py
  python3 camera/test_image_publisher.py   ← 테스트 카메라
══════════════════════════════════════════════════════════════════
"""
...
class JackalUR10GroundDemo:
    """Jackal + UR10 지상 테스트 (수중 환경 없음)"""
    ...
    def setup(self):
        print("=" * 60)
        print("JACKAL + UR10  Ground Test Demo")
        print("=" * 60)
        ...
        print("\n✓ Setup complete (ground mode - 수중 환경 없음)")
    ...
    def _create_weld_path(self, stage):
        ...
        print("  ✓ 용접 경로 시각화 (노란 경로선 + 순번 구체 + 흰 인디케이터)")
    ...
    async def run_async(self):
        ...
        print("\n[Phase 1] HOME 포지션")
        await self._move_arm(HOME_POSE_DEG, 4.0)

        print("\n[Phase 2] Reach 자세 → 균열 추종 시작")
        await self._move_arm(REACH_POSE_DEG, 3.0)
